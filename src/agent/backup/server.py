...
def import_example() -> None:
    ...
    # Apply the monkey patch to the graph to enable OpenAI compatibility
    app.agent.graph = original_graph
...
# This middleware function will help ensure OpenAI compatibility by preprocessing messages
def preprocess_graph_state(state):
    """
    Preprocess the state's messages to ensure OpenAI compatibility
    This is a multi-layered approach - this fixes messages in the state,
    and the DirectOpenAIFix will catch anything that slips through
    """
    if not state or "messages" not in state:
        return state
    
    # Direct access to state messages
    messages = state["messages"]
    
    # Fix any null content in messages
    for msg in messages:
        if hasattr(msg, 'content') and msg.content is None:
            msg.content = ""
    
    # Check all assistant messages for tool_calls
    processed_messages = []
        ...
        if hasattr(msg, 'tool_calls') and msg.tool_calls:
            for tc in msg.tool_calls:
                if isinstance(tc, dict) and 'id' in tc:
                    tool_calls[tc['id']] = True
    
    # Second pass: collect all tool responses
    tool_responses = {}
    for msg in messages:
        if hasattr(msg, 'tool_call_id') and msg.tool_call_id:
            tool_responses[msg.tool_call_id] = True
    
    # Third pass: add messages with responses where needed
    for i, msg in enumerate(messages):
        ...
        if (hasattr(msg, 'role') and msg.role == 'assistant' or 
            msg.__class__.__name__ == 'AIMessage') and hasattr(msg, 'tool_calls') and msg.tool_calls:
            
            # For each tool call, ensure there's a tool message response
            for tc in msg.tool_calls:
                if isinstance(tc, dict) and 'id' in tc:
                    tc_id = tc['id']
                    if tc_id and tc_id not in tool_responses:
                        logger.info(f"Adding missing tool response for tool_call_id: {tc_id}")
                        from langchain_core.messages import ToolMessage
                        processed_messages.append(ToolMessage(
                            tool_call_id=tc_id,
                            content="No result available for this tool call"
                        ))
                        tool_responses[tc_id] = True
    
    # Final check for any remaining tool calls without responses
    missing_responses = [tc_id for tc_id in tool_calls if tc_id not in tool_responses]
    if missing_responses:
        logger.warning(f"Still found {len(missing_responses)} tool calls without responses after main processing")
        for tc_id in missing_responses:
            from langchain_core.messages import ToolMessage
            processed_messages.append(ToolMessage(
                tool_call_id=tc_id,
                content="Emergency generated tool response"
            ))
    
    if len(processed_messages) != len(messages):
        logger.info(f"Preprocessed messages for OpenAI compatibility: {len(messages)} → {len(processed_messages)}")
        state = {**state, "messages": processed_messages}
    
    return state

# Create a custom subclass of StateGraph that adds preprocessing
original_graph = graph

# Monkey patch the graph's astream_events method to preprocess state
original_astream_events = original_graph.astream_events

async def patched_astream_events(self, *args, **kwargs):
    # Get the input state from args
    if args and isinstance(args[0], dict) and "messages" in args[0]:
        # Preprocess the input state
        args = list(args)
        args[0] = preprocess_graph_state(args[0])
        args = tuple(args)
    
    # Call the original method but intercept all states yielded
    async for event in original_astream_events(*args, **kwargs):
        if event.get("event") == "on_chain_start" and "data" in event and "input" in event["data"]:
            input_data = event["data"]["input"]
            if isinstance(input_data, dict) and "messages" in input_data:
                # Preprocess each state
                event["data"]["input"] = preprocess_graph_state(input_data)
                
        yield event

# Apply the monkey patch
original_graph.astream_events = patched_astream_events.__get__(original_graph, type(original_graph))
    ...
    try:
        ...
        async def response_generator():
            ...
            try:
                ...
                # Preprocess chat history in case it contains tool calls without responses
                chat_history = app.session_manager.get_conversation(prompt.session_id)
                if chat_history:
                    logger.info(f"Processing chat history with {len(chat_history)} messages for OpenAI compatibility")
                    # Format chat history into state-like format so we can use preprocess_graph_state
                    ...
                    processed_history_state = preprocess_graph_state(history_state)
                    # Update config if preprocessing changed anything
                    if len(processed_history_state["messages"]) != len(history_state["messages"]):
                        logger.info(f"Chat history preprocessing added {len(processed_history_state['messages']) - len(history_state['messages'])} messages")
                        config["configurable"]["chat_history"] = processed_history_state["messages"]
                ...
                # Apply preprocessing to ensure OpenAI compatibility before sending to the graph
                if input_for_graph and "messages" in input_for_graph:
                    logger.info(f"Input for graph before preprocessing: {input_for_graph}")
                    input_for_graph = preprocess_graph_state(input_for_graph)
                    logger.info(f"Input for graph after preprocessing: {input_for_graph}")
