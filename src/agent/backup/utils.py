...
def create_tool_node_with_fallback(tools: list) -> dict:
    """Creates a ToolNode with proper error handling that ensures OpenAI-compatible tool message responses"""
    # Create a standard ToolNode to execute the tool
    tool_node = ToolNode(tools).with_fallbacks(
        [RunnableLambda(handle_tool_error)], exception_key="error"
    )
    
    # Create a wrapper to ensure that tool invocations always result in proper tool messages
    def ensure_tool_messages(state):
        # Execute the tool node
        result = tool_node.invoke(state)
        
        # If the last message has tool_calls but we don't have ToolMessages in the result
        if (state["messages"] and 
            ...
            # If there are no tool messages or fewer tool messages than tool calls
            if not any(isinstance(m, ToolMessage) for m in messages_result) or \
               len([m for m in messages_result if isinstance(m, ToolMessage)]) < len(tool_calls):
                # For each tool call, ensure there's a corresponding ToolMessage
                for tc in tool_calls:
                    # Check if there's already a ToolMessage for this tool_call_id
                    if not any(isinstance(m, ToolMessage) and hasattr(m, "tool_call_id") and m.tool_call_id == tc["id"] 
                              for m in messages_result):
                        # If no ToolMessage exists for this tool_call_id, create one with a default message
    ...
    # Return a lambda that wraps the tool node
    return RunnableLambda(ensure_tool_messages)
