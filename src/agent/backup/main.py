...
class Assistant:
    ...
    async def __call__(self, state: State, config: RunnableConfig):
        while True:
            ...
            # Check for OpenAI compatibility - ensure any tool call has a corresponding tool response
            # Only AIMessage can have tool_calls, so check the type first
            if (hasattr(last_message, 'tool_calls') and 
                last_message.tool_calls and 
                not any(
                    isinstance(m, ToolMessage) and hasattr(m, 'tool_call_id') and m.tool_call_id == tc["id"]
                    for m in state["messages"][-len(last_message.tool_calls):]
                    for tc in last_message.tool_calls
                )):
                # Get results for all tool calls
                result_messages = []
                for tc in last_message.tool_calls:
            ...
            if not hasattr(result, 'tool_calls') or not result.tool_calls and (
                not result.content
        ...
        # The result should be wrapped in a list for the add_messages reducer
