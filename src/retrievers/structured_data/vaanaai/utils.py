...
class OpenAIEmbeddingsWrapper:
    """Wrapper for OpenAI embeddings to use with Vanna library"""
    ...
    def encode_queries(self, queries: List[str]) -> List[np.array]:
        ...
        # return [np.array(embedding) for embedding in self.openai_embeddings.embed_query(queries)]
        return list(map(np.array, [self.openai_embeddings.embed_query(query) for query in queries]))
